...
\title{\textbf{Kineforge: Sample-Efficient Embodied Control with Frozen Semantic Priors and Differentiable Physics}\\[0.4em]
\large Technical Report v1.0 --- Brightforge Software Inc.}
\author{Illia Ovcharenko \\[0.3em] \textit{Brightforge Software Inc.} \\ \textit{Kineforge AI-Lab.} \\ \texttt{illia.ovcharenko@brightforge.ltd}}
\date{May 2026}
...
We argue that \textbf{structured inductive bias}---not parameter count---is the lever for sample-efficient embodied control. \textbf{Kineforge} is a lightweight stack that couples (i) \textbf{differentiable physics simulation} (MJX) for gradient-friendly rollouts, (ii) a \textbf{multi-stream policy} with parallel reflex, affect, and planning pathways, and (iii) a \textbf{frozen semantic prior} encoding domain structure without online supervision. The trainable policy is compact (${\sim}$157K parameters); priors remain fixed (${\sim}$2.6M parameters), decoupling representation learning from control learning.
...
RL with foundation priors (RLFP) demonstrates that frozen value and success models can accelerate real-robot learning~\cite{ye2024rlfp}. Kineforge differs by using \emph{structured symbolic priors} rather than general VLMs, targeting edge deployability.
...
Figure~\ref{fig:architecture} illustrates Kineforge. Per timestep, the agent encodes observations, retrieves a frozen semantic embedding, and routes decisions through three streams before actuating the differentiable simulator.
...
\begin{figure}[t]
  ...
  \vspace{2.8em}}}
  \caption{Kineforge control stack. Public diagrams omit proprietary graph connectivity and prior token identities.}
...
Kineforge is implemented in Python with JAX (no PyTorch). Orchestration uses SkyPilot on Kubernetes (Nebius H200). Metrics (steps per second, homeostasis, conflict rate) log to MLflow. We withhold source-level module names, graph construction rules, and prior databases.
...
Training runs on NVIDIA H200 (MIG slices, CUDA 13) with containerized dependencies (\texttt{.venv-nebius-mjx-cuda13}). Object storage uses Nebius \texttt{us-central1} (\texttt{storage.us-central1.nebius.cloud}); MLflow day reports and US \texttt{bootstrap/} archives hold fresh runs, while legacy \texttt{eu-north/} snapshots are retained for comparison. Monolithic jobs target up to $1.5{\times}10^6$ environment steps; A/B studies use $10^4$--$10^5$ steps for controlled comparisons.
...
\texttt{jax\_kineforge\_runtime.py} logs SPS, homeostasis variance, conflict rate, food concentration, nutrition gap, and discrete state index.

\paragraph{Baselines.}
We report DreamerV3 configurations present in the repository as exploratory baselines; primary claims use Kineforge multi-stream runs.
...
\begin{figure}[H]
  \centering
  \IfFileExists{figures/fig2_throughput_vs_task_reward.png}{%
    \includegraphics[width=0.95\linewidth]{fig2_throughput_vs_task_reward.png}}{%
    \fbox{\parbox{0.9\linewidth}{\centering Missing fig2 PNG in figures/}}}
  \caption{Throughput vs.\ task reward across Nebius compare runs (38 logged A/B jobs).}
  \label{fig:scatter}
\end{figure}
...
\begin{figure}[H]
  \centering
  \IfFileExists{figures/fig5_learning_panel.png}{%
    \includegraphics[width=\linewidth]{fig5_learning_panel.png}}{%
    \IfFileExists{figures/fig5_homeostasis_curve.png}{%
      \includegraphics[width=\linewidth]{fig5_homeostasis_curve.png}}{%
      \fbox{\parbox{0.9\linewidth}{\centering Missing fig5 PNG in figures/}}}}
  \caption{MLflow homeostasis (left) and conflict rate (right) vs.\ training steps (monolithic H200 run).}
  \label{fig:learning}
\end{figure}

\paragraph{Long-horizon trends.}
MLflow \texttt{day\_block} logs from a monolithic H200 run (${\sim}$5K--$1.1{\times}10^6$ steps, 221 checkpoints) show homeostasis stabilizing near ${\sim}$0.57 and conflict rate declining from ${\sim}$1.0 toward ${\sim}$0.67 (Figure~\ref{fig:learning}). Baseline task reward remains near 0.70 for horizons $10^4$--$10^5$ steps in A/B compares; at $2{\times}10^5$ steps some runs show lower task reward (${\sim}$0.56--0.70), motivating future curriculum and replay tuning (Appendix).
...
Unlike billion-parameter VLA models, Kineforge prioritizes \textbf{structure over scale}: frozen priors plus compact $\theta$ enable training in hours on a single GPU rather than weeks on clusters.
...
Kineforge demonstrates that embodied agents can train without human demonstrations when structured semantic priors and differentiable physics are combined with a multi-stream policy. Future work targets real-robot pilots, public inference APIs, and open benchmarks with sanitized priors.
